...
public class Sjf extends Escalonador{
	...
	public Sjf(String[][] array, int tamanhoArray) {
		...
		do {
			for(int x = 0; x <tamanho; x++) {
				if(vetorAuxiliar[x][1] == auxChegada[cont]) {
					...
					ct[x] = finalExecucao;
					//Se o tempo de execucao chegar a 0(zero), o processo termina
					if(contX == 0) {
						finalExecucao = vetorAuxiliar[x][1]+vetorAuxiliar[x][2];
						saidaGrafico[contX][1] = String.format("    %d ", finalExecucao);
						ct[x] = finalExecucao;
					}else {
						//se o tempo de execução for maior que 1
						if(vetorAuxiliar[x][1] == vetorAuxiliar[x][2]) {
							dAt = vetorAuxiliar[x][1] - vetorAuxiliar[x][2];
							finalExecucao = dAt;
							saidaGrafico[contX][1] = String.format("   %d ", finalExecucao);
							ct[x] = finalExecucao;
						}
